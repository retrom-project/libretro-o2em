...
with tempfile.TemporaryDirectory() as temporary:
    ...
    if json.loads((extracted / "core.json").read_text())["name"] != core:
        raise SystemExit("RETROM_CORE_ARCHIVE_INVALID")
    if not (extracted / "license.txt").read_bytes() == (args.output / license_name).read_bytes():
        raise SystemExit("RETROM_CORE_LICENSE_INVALID")
    if not (args.output / license_name).read_bytes().startswith((ROOT / "COPYING").read_bytes()):
        raise SystemExit("RETROM_CORE_LICENSE_INVALID")
